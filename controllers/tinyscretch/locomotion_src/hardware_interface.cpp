...
void readAllMotorPos(robotTypeDef* rob, float dt)
{
    ...
    DigitalLPFw(LIMIT(To_180_degreesw(rob->arm_base_height- rob->q_reg_based_height) / dt, -0.5, 0.5),
            &rob->arm_base_dheight, 100, dt);
    //printf("div_z:z=%.3f dz=%.3f z_o=%.3f\n",rob->arm_base_height,rob->arm_base_dheight,leg_motor[0].q_now[0]);
...
void set_motor_arm(void)//机械臂控制 input degree
{
    ...
    float out_flt[6]={0.3,0.3,0.375,0.375,0.375,0.5};
...
//            robotwb.arm_q_exp_flt[0][4]);
    ...
    float exp_u = LIMIT((robotwb.arm_base_height_exp_flt - arm_base_height) * 8,-0.45,0.45);
    if(robotwb.state_gait==1)//auto down
        exp_u=-0.3;
...
void updateJoy(void)//
{
    ...
    if(ocu.mode==RC_REMOTE_UP){//auto mode
    ...
    if(robotwb.state_gait>=2){
        ...
        if(robotwb.arm_control_mode==ARM_M_H){
                ...
                if(ocu.key_ll)
                     robotwb.arm_epos_b_exp.z-=0.1*0.005;
                if(ocu.key_rr)
                     robotwb.arm_epos_b_exp.z+=0.1*0.005;
                ...
                //yaw
                robotwb.arm_att_b_exp.z+=-ocu.rc_att_w[1]*25*0.005;
                robotwb.arm_att_b_exp.z=LIMIT(robotwb.arm_att_b_exp.z,-45,45);

                robotwb.arm_att_b_exp.x+=ocu.rate_yaw_w*50*0.005;
            ...
            if(ocu.key_ll)
                 robotwb.arm_epos_e_exp.z-=0.1*0.005;
            if(ocu.key_rr)
                 robotwb.arm_epos_e_exp.z+=0.1*0.005;
            ...
            //yaw
            robotwb.arm_att_n_exp.z+=-ocu.rc_att_w[1]*25*0.005;
            robotwb.arm_att_n_exp.z=LIMIT(robotwb.arm_att_n_exp.z,-45,45);

            robotwb.arm_att_n_exp.x+=ocu.rate_yaw_w*50*0.005;
