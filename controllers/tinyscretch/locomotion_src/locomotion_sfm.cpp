...
void vmc_param_init(void)
{
    ...
    robotwb.dq_2_dv_base=0.0008;//<--------------doghome
    ...
    robotwb.min_epos_z=-0.08;
    robotwb.min_q_z=0.02;
    for(int i=0;i<4;i++)
    {
...
void reset_tar_pos(char sel,char init_q) {//区分着地离地？<<----------------------------!!!!!!!!!!!!!!!!
    ...
    robotwb.arm_att_h_exp_flt=robotwb.arm_att_h_exp=robotwb.arm_att_h;
...
void locomotion_sfm(float dt)
{
   ...
   case 1://------------
...
#else
            float gain_all=1;
            if(fabs(robotwb.arm_base_tau)>35)
                temp_flag[0]+=1;
#endif
            temp_flag[0]+=move_joint_to_pos_sel1(&robotwb,0,robotwb.safe_arm_q[0][0],250*gain_all,5,0,dt);//d
            temp_flag[0]+=move_joint_to_pos_sel1(&robotwb,1,robotwb.safe_arm_q[0][1],500*gain_all,5,0,dt);//x
       ...
       if(temp_flag[0]==6){//可强制结束 Lock now angle
           ...
           leg_motor[0].q_bias[0]=leg_motor[0].q_now[0]-0.03;
#if 1
